...
import { ArrowRight, Check, Hospital, Pill, FlaskConical, Stethoscope, Bot, ShieldCheck, BookOpen, Headphones, Cloud } from "lucide-react";
...
export const Route = createFileRoute("/nuronix")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Open-source, web-based EMR and HMIS system for hospitals, clinics, labs, and pharmacies in Nepal." },
...
const features = {
  web: {
    ...
    icon: Cloud,
    list: ["Zero installation", "Works on Chrome / Firefox / Safari", "Mobile responsive", "Multi-branch support", "Real-time sync"],
  },
  security: {
...
const modules = [
  {
    ...
    list: ["Drug inventory management with low-stock alerts", "Prescription-to-dispense workflow integration", "Drug interaction and allergy alerts", "Batch tracking and expiry management", "Supplier and purchase order management", "Daily, weekly, and monthly sales reports", "Barcode scanning support", "Controlled substance tracking"],
  },
  {
    icon: FlaskConical,
    ...
    list: ["Doctor-to-lab test order integration", "Lab technician workflow management", "Reference range flagging (normal/abnormal)", "Automated result delivery to patient record", "Lab report PDF generation and printing", "Specimen tracking and chain of custody", "Haematology, biochemistry, microbiology, radiology", "Integration with external LIS systems"],
  },
  {
    icon: Stethoscope,
    ...
    list: ["Online and walk-in appointment booking", "Digital queue display system", "Consultation room workflow", "Chief complaint and SOAP note templates", "ICD-10 diagnosis code support", "Follow-up scheduling with automated reminders", "Billing and invoice generation", "Patient satisfaction scoring"],
  },
  {
    icon: Hospital,
    name: "EMR Core",
    ...
    list: ["Patient registration and unique health ID", "Complete clinical history", "IPD and OPD management", "Bed and ward management", "Nursing notes and care plans", "Surgical and procedure records", "Discharge summary generation", "Referral management"],
  },
  {
    icon: Bot,
    ...
    list: ["AI-powered diagnosis suggestion engine", "Readmission risk prediction", "Automated HMIS reporting to government portals", "Pattern detection for disease outbreaks", "Smart appointment scheduling optimization", "Natural language clinical note generation"],
  },
];
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-brand-gradient text-white">
        ...
        <div className="container mx-auto px-4 md:px-6 py-20 md:py-28 relative text-center max-w-4xl">
          ...
          <p className="mt-5 text-lg text-white/85 max-w-3xl mx-auto">
            An open-source, web-based HMIS and EMR platform designed specifically for Nepal's healthcare system. Nuronix brings clinical workflows, patient records, and hospital operations under one intelligent roof.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link to="/contact" className="inline-flex items-center gap-2 rounded-xl bg-white text-brand-royal px-6 py-3.5 font-semibold hover:bg-brand-soft transition">
              Request a Demo <ArrowRight className="h-4 w-4" />
            </Link>
            <a href="#modules" className="inline-flex items-center gap-2 rounded-xl border-2 border-white/70 text-white px-6 py-3.5 font-semibold hover:bg-white/10 transition">
              View Documentation
            </a>
          </div>
            ...
            <div className="relative rounded-2xl bg-white text-foreground shadow-elegant p-4 text-left">
              <div className="flex gap-2 mb-3">
                {["Dashboard", "Patients", "OPD", "Lab", "Pharmacy", "Reports"].map((t, i) => (
                  <span key={t} className={`text-xs px-3 py-1.5 rounded-md ${i === 0 ? "bg-brand-royal text-white" : "bg-muted text-muted-foreground"}`}>{t}</span>
                ))}
              </div>
              <div className="grid grid-cols-4 gap-3">
                {["Active OPD", "Inpatients", "Lab Pending", "Revenue (NPR)"].map((l, i) => (
                  <div key={l} className="rounded-lg border p-3">
                    ...
                    <p className="font-extrabold text-brand-deep mt-1">{["142", "38", "21", "1.2L"][i]}</p>
                  </div>
                ))}
              </div>
              <div className="mt-3 rounded-lg bg-brand-soft border p-3 h-24 flex items-end gap-1">
                {[35, 60, 45, 75, 55, 85, 70, 92, 60, 78].map((h, i) => (
                  <div key={i} className="flex-1 bg-gradient-to-t from-brand-royal to-brand-electric rounded-sm" style={{ height: `${h}%` }} />
                ))}
              </div>
            </div>
          </div>
      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div>
            <SectionHeader center={false} eyebrow="What is Nuronix?" title="Built for Nepal. Built to WHO Standard." />
            <div className="space-y-4 text-muted-foreground text-lg">
              <p>
                Nuronix is Growfund's open-source Electronic Medical Record (EMR) and Hospital Management Information System (HMIS), developed in full compliance with the World Health Organization's (WHO) standards for health informatics.
              </p>
              <p>
                It is Nepal's first truly customizable, community-driven EMR — built to address the specific clinical, administrative, and regulatory needs of Nepali healthcare facilities.
              </p>
              <p>
                Whether you run a 10-bed clinic in Pokhara or a 500-bed tertiary hospital in Kathmandu, Nuronix adapts to your workflow — not the other way around.
              </p>
            </div>
          </div>
          <div className="rounded-2xl border bg-brand-soft-gradient p-8 shadow-soft">
            ...
            <dl className="space-y-4">
              {[
                ["📋 Type", "EMR + HMIS"],
                ["🌐 Deployment", "Web-based (browser only, no installation)"],
                ["🔓 License", "Open Source (Apache 2.0)"],
                ["🏛️ Standard", "WHO HMIS Framework"],
                ["🇳🇵 Origin", "Made in Nepal"],
              ].map(([k, v]) => (
                <div key={k} className="flex justify-between gap-4 pb-3 border-b border-brand-royal/10 last:border-0">
        ...
        <Tabs defaultValue="web" className="max-w-5xl mx-auto">
          <TabsList className="grid grid-cols-3 w-full max-w-2xl mx-auto h-auto p-1.5 bg-white border shadow-soft">
            {Object.entries(features).map(([key, f]) => (
              <TabsTrigger key={key} value={key} className="py-3 data-[state=active]:bg-brand-gradient data-[state=active]:text-white text-sm font-semibold">
                {f.title.split(" ")[0]}
              </TabsTrigger>
            ))}
          </TabsList>
      ...
      {/* Modules */}
      <Section id="modules">
        <SectionHeader eyebrow="Module Deep-Dive" title="Everything Your Hospital Needs. All in One System." />
        <Accordion type="single" collapsible defaultValue="0" className="max-w-4xl mx-auto">
          {modules.map((m, i) => (
            <AccordionItem key={m.name} value={String(i)} className="border rounded-2xl mb-3 px-6 bg-card shadow-soft">
      ...
      {/* Who is it for */}
      <Section variant="soft">
        <SectionHeader eyebrow="Who Is Nuronix For?" title="Designed for Every Corner of Nepal's Healthcare" />
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            ...
            <div key={t} className="rounded-2xl border bg-card p-6 shadow-soft text-center hover:shadow-elegant hover:-translate-y-1 transition">
      ...
      <Section>
        <SectionHeader eyebrow="Implementation Journey" title="From Contract to Go-Live in 14 Days" />
      ...
      <Section variant="soft">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {[
            { n: "Community", p: "Free", d: "Core EMR · 1 facility · community support", f: ["Nuronix Core", "Self-hosted", "Community forum"] },
            { n: "Professional", p: "Contact us", d: "All modules · cloud hosting · priority support", featured: true, f: ["All modules", "Cloud hosting", "Priority support", "Training included"] },
            { n: "Enterprise", p: "Custom", d: "Multi-branch · SLA · dedicated team · custom dev", f: ["Multi-branch", "SLA & dedicated team", "Custom development", "On-site support"] },
          ].map((p) => (
            <div key={p.n} className={`rounded-2xl p-8 shadow-soft ${p.featured ? "bg-brand-gradient text-white scale-105 shadow-elegant" : "bg-card border"}`}>
              <p className={`text-sm font-semibold uppercase tracking-wider ${p.featured ? "text-white/80" : "text-brand-royal"}`}>{p.n}</p>
              <p className={`mt-3 text-4xl font-extrabold ${p.featured ? "text-white" : "text-brand-deep"}`}>{p.p}</p>
              <p className={`mt-2 text-sm ${p.featured ? "text-white/80" : "text-muted-foreground"}`}>{p.d}</p>
              <ul className="mt-6 space-y-2">
                {p.f.map((x) => (
                  <li key={x} className="flex items-center gap-2 text-sm">
                    <Check className={`h-4 w-4 ${p.featured ? "text-white" : "text-brand-royal"}`} /> {x}
                  </li>
                ))}
              </ul>
              <Link to="/contact" className={`mt-6 block text-center rounded-xl px-5 py-3 font-semibold transition ${p.featured ? "bg-white text-brand-royal hover:bg-brand-soft" : "bg-brand-gradient text-white hover:opacity-90"}`}>
                Talk to Us
              </Link>
            </div>
          ))}
